...
| `--stt-extra-body`                      | `STT_EXTRA_BODY`                           | None                                          | JSON object merged into the STT request body via `extra_body` for backend-specific fields. Overlapping keys override top-level request fields. |
...
| `--tts-extra-body`                      | `TTS_EXTRA_BODY`                           | None                                          | JSON object merged into the TTS request body via `extra_body` for backend-specific fields. Overlapping keys override top-level request fields. |
...
Both `STT_EXTRA_BODY` and `TTS_EXTRA_BODY` must be valid JSON objects. The OpenAI client merges these values into the outgoing request body rather than sending a nested `extra_body` field. If an `extra_body` key overlaps with a populated top-level request field such as `response_format`, `speed`, `instructions`, or `stream`, the `extra_body` value takes precedence. For example, use `TTS_EXTRA_BODY='{"stream": true}'` to forward a backend-specific `stream` flag or override the top-level setting when needed.
...
- **`major.minor.patch version`**: Specific version tags (e.g., `0.4.1`) correspond to specific stable releases of the Wyoming OpenAI proxy server. These tags are ideal for users who need a consistent, reproducible environment and want to avoid breaking changes introduced in newer versions.
