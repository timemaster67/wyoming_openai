...
| `--stt-backend`                         | `STT_BACKEND`                              | None (autodetected)                           | Enable unofficial API feature sets such as `OPENAI`, `OPENROUTER`, `SPEACHES`, `KOKORO_FASTAPI`, or `LOCALAI`. If `STT_OPENAI_URL` points to `openrouter.ai`, the OpenRouter backend is autodetected and uses JSON base64 audio transcription requests. |
